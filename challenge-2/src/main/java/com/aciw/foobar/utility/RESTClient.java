...
public class RESTClient {
	...
	public static void sendOutput(ArrayList<String> answer) {
		try {
			...
			WebTarget webTarget = client.target(baseUrl + "challenges/Decryption/submit");
	...
	public static ArrayList<Decryption> getInput(int set_id) {
		...
		// service.
		String baseUrl = retrieveUrl();
		WebTarget webTarget = client.target(baseUrl + "challenges/Decryption/"+set_id);
		String authValue = Base64.getEncoder().encodeToString((username + ":" + password).getBytes());
		Invocation.Builder invocationBuilder = webTarget.request(MediaType.APPLICATION_JSON).header("Authorization",
				"Basic " + authValue);
		...
		if (checkLoginPassed(rspStr)) {
			...
			ArrayList<Decryption> result = response.readEntity(new GenericType<ArrayList<Decryption>>() {
			});
			return result;
		} else {
			// Login failed
